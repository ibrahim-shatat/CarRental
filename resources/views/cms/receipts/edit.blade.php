...
@section('main-title' , 'Edit Receipt')

@section('sub-title' , 'Edit Receipt')
        ...
        <div class="col-md-12">
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Edit Data of Receipt</h3>
            </div>
            <form>
              <div class="card-body">
                <div class="form-group">
                    <label for="name">Receipt Name</label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ $receipts->name }}" placeholder="Enter Name of Receipt">
                </div>
                <div class="form-group">
                    <label for="receipt_time">Receipt Time</label>
                    <input type="time" class="form-control" id="receipt_time"value="{{ $receipts->receipt_time }}" name="receipt_time" placeholder="Enter Time of Receipt">
                </div>
                <div class="form-group">
                    <label for="receipt_date">Receipt Date</label>
                    <input type="date" class="form-control" id="receipt_date" name="receipt_date" value="{{ $receipts->receipt_date }}" placeholder="Enter Date of Receipt">
                </div>
                <div class="form-group col-md-6">
                    <label for="payment_method"> Paymernt Method</label>
                    <select class="form-select form-select-sm" name="payment_method" style="width: 100%;" value="{{ $receipts->payment_method }}"
              ...
              <div class="card-footer">
                <button type="button" onclick="performUpdate({{ $receipts->id  }})" class="btn btn-primary">Update</button>
...
<script>
    function performUpdate(id){
        ...
        storeRoute('/cms/admin/receipts_update/'+id,formData);
    }
